...
vals = {"econ":"ordo,opt,soc,marksoc,cap,socdem,clas,com", "gov":"repub,auth,rep,techno,dir,plut,strato,anar,rep*,dir*,repub*,auth*","soct":"perf,free,prog,trad,just","rel": "theo,hybr,athe,sec,athe*","for":"sov,asim,nat,open,iso,pop,glob,glob*,open*,nat*","legal":"proced,decn,lao,hum,hum*,prot,prot*"}
for i in vals:
    vals[i] = vals[i].split(",")
    keysused.append(i)
    for i in vals[i]:
